...
function animate_cycle_original_fixed(fig, t, z_all, walker, cycle_num, speed_factor)
    ...
    % === 脚のアイデンティティを完全固定（プログラム実行中は絶対に変更しない） ===
    persistent global_q1_identity_fixed
    
    % 最初の実行時のみアイデンティティを決定
    if isempty(global_q1_identity_fixed)
        % 初期フレームで脚の位置関係を判定
        q1_init = z_all(1,1);
        q2_init = z_all(1,3);
        if size(z_all, 2) >= 6
        ...
        % 初期の足先位置で判定
        leg1_foot_x_init = xh_init + (l+r)*sin(q1_init);
        leg2_foot_x_init = xh_init + (l+r)*sin(-(q2_init-q1_init));
        
        if leg1_foot_x_init > leg2_foot_x_init
            global_q1_identity_fixed = 'right';  % q1 = 右脚
            fprintf('永続的脚アイデンティティ設定: q1=右脚(赤), q2=左脚(青)\n');
        else
            global_q1_identity_fixed = 'left';   % q1 = 左脚
            fprintf('永続的脚アイデンティティ設定: q1=左脚(青), q2=右脚(赤)\n');
        end
    end
    
    % 固定された色設定（以後絶対に変更しない）
    if strcmp(global_q1_identity_fixed, 'right')
        q1_color_fixed = 'red';    % q1 = 右脚（赤）
        q2_color_fixed = 'blue';   % q2 = 左脚（青）
    else
        q1_color_fixed = 'blue';   % q1 = 左脚（青）
        q2_color_fixed = 'red';    % q2 = 右脚（赤）
    end
    ...
    % 脚オブジェクトの初期化（固定色で設定）
    barpic(1) = line('xdata',barref(1,:),'ydata',barref(2,:),'linewidth', 3, 'color', q1_color_fixed);
    barpic(2) = line('xdata',barref(1,:),'ydata',barref(2,:),'linewidth', 3, 'color', q2_color_fixed);
    ...
    title(sprintf('サイクル %d - 歩行アニメーション（脚色完全固定）', cycle_num), 'FontSize', 14)
    ...
    for i = 1:mm
        try
            ...
            idx = find(t <= t_anim(i));
            if ~isempty(idx)
                plot(t(idx), z_all(idx,1), '-', 'Color', q1_color_fixed, 'LineWidth', 2, 'DisplayName', 'q1');
                plot(t(idx), z_all(idx,3), '-', 'Color', q2_color_fixed, 'LineWidth', 2, 'DisplayName', 'q2');
                plot(t(idx), z_all(idx,2), '--', 'Color', q1_color_fixed, 'LineWidth', 1, 'DisplayName', 'u1');
                plot(t(idx), z_all(idx,4), '--', 'Color', q2_color_fixed, 'LineWidth', 1, 'DisplayName', 'u2');

                % 現在位置をマーク
                if i <= length(z)
                    plot(t_anim(i), z(i,1), 'o', 'Color', q1_color_fixed, 'MarkerSize', 8, 'MarkerFaceColor', q1_color_fixed);
                    plot(t_anim(i), z(i,2), 'o', 'Color', q2_color_fixed, 'MarkerSize', 8, 'MarkerFaceColor', q2_color_fixed);
                end
            end
...
function animate_all_cycles_original_fixed(fig, all_times, all_extended_states, walker, speed_factor)
    ...
    % === 脚のアイデンティティを完全固定（プログラム実行中は絶対に変更しない） ===
    persistent global_all_cycles_q1_identity_fixed
    ...
    % 最初の実行時のみアイデンティティを決定
    if isempty(global_all_cycles_q1_identity_fixed)
        % 初期フレームで脚の位置関係を判定
        q1_init = z_all(1,1);
        q2_init = z_all(1,3);
        if size(z_all, 2) >= 6
        ...
        if leg1_foot_x_init > leg2_foot_x_init
            global_all_cycles_q1_identity_fixed = 'right';  % q1 = 右脚
            fprintf('全サイクル永続的脚アイデンティティ設定: q1=右脚(赤), q2=左脚(青)\n');
        else
            global_all_cycles_q1_identity_fixed = 'left';   % q1 = 左脚
            fprintf('全サイクル永続的脚アイデンティティ設定: q1=左脚(青), q2=右脚(赤)\n');
        end
    end
    
    % 固定された色設定（以後絶対に変更しない）
    if strcmp(global_all_cycles_q1_identity_fixed, 'right')
        q1_color_fixed = 'red';    % q1 = 右脚（赤）
        q2_color_fixed = 'blue';   % q2 = 左脚（青）
    else
        q1_color_fixed = 'blue';   % q1 = 左脚（青）
    ...
    for p = 1:2
        if p == 1
            barpic(p)=line('xdata',barref(1,:),'ydata',barref(2,:),'linewidth', 3, 'color', q1_color_fixed);
        else
            barpic(p)=line('xdata',barref(1,:),'ydata',barref(2,:),'linewidth', 3, 'color', q2_color_fixed);
        end
    end
    ...
    title('全サイクル 歩行アニメーション（脚色完全固定）', 'FontSize', 14)
    
    % サブプロット2: 状態変数
    subplot(1,2,2);
    title('全サイクル 状態変数（脚色完全固定）');
    xlabel('時間 [s]');
    grid on;
    hold on;
    ...
    for i = 1:mm
        try
            ...
            idx = find(t_all <= t(i));
            if ~isempty(idx)
                plot(t_all(idx), z_all(idx,1), '-', 'Color', q1_color_fixed, 'LineWidth', 2, 'DisplayName', 'q1');
                plot(t_all(idx), z_all(idx,3), '-', 'Color', q2_color_fixed, 'LineWidth', 2, 'DisplayName', 'q2');
                plot(t_all(idx), z_all(idx,2), '--', 'Color', q1_color_fixed, 'LineWidth', 1, 'DisplayName', 'u1');
                plot(t_all(idx), z_all(idx,4), '--', 'Color', q2_color_fixed, 'LineWidth', 1, 'DisplayName', 'u2');

                % 現在位置をマーク
                if i <= length(z)
                    plot(t(i), z(i,1), 'o', 'Color', q1_color_fixed, 'MarkerSize', 8, 'MarkerFaceColor', q1_color_fixed);
                    plot(t(i), z(i,2), 'o', 'Color', q2_color_fixed, 'MarkerSize', 8, 'MarkerFaceColor', q2_color_fixed);
                end
            end
