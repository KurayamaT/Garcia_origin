...
q1 = 0.0;  u1 = -0.1;  % スタンス脚に後ろ向きの初速度
q2 = 0.2;  u2 = -0.1;  % スイング脚にも初速度
...
[z,t] = onestep(z0,walker,steps);      % 修正版（初期値から）
...
% disp('Saving animation data to CSV...');
% save_animation_data_extended(t,z,walker,steps,fps);
...
%===================================================================
function save_animation_data_extended(t_all,z_all,walker,steps,fps)
%===================================================================
% This function saves all animation data including foot segments to CSV

%%%% Interpolate linearly using fps (same as animate function) %%%%%
z_all_plot = [z_all(:,1) z_all(:,3) z_all(:,6) z_all(:,8)];
nn = size(z_all_plot,2);
total_frames = round(t_all(end)*fps);
t = linspace(0,t_all(end),total_frames);
...
mm = size(z,1);
lines_for_feet = 4;
th = 0.25; % angle for foot segments

% Initialize data arrays
basic_data = zeros(mm, 13); % Basic animation data
foot_data = zeros(mm, lines_for_feet*2*4); % 4 segments per foot, 2 feet, 4 coords per segment
...
for i=1:mm
    ...
    % Store basic data
    basic_data(i,:) = [t(i), xh, yh, ...
                      bar(1,1,1), bar(2,1,1), bar(1,2,1), bar(2,2,1), ... % stance leg
                      bar(1,1,2), bar(2,1,2), bar(1,2,2), bar(2,2,2), ... % swing leg
                      q1, q2];
    
    % Calculate foot segments
    B = [-th -2*th; 0 -th; th 0; 2*th th]; % angles for foot segments
    
    foot_idx = 1;
    for p=1:2 % for each foot
        for q=1:4 % for each segment
            C = A(p) + B(q,:);
            foot_segment = [center(:,:,p), center(:,:,p)] + r*R(C(1))*[O,y] + r*R(C(2))*[y, O];
            
            % Store foot segment coordinates
            foot_data(i,foot_idx:foot_idx+3) = [foot_segment(1,1), foot_segment(2,1), ...
                                                foot_segment(1,2), foot_segment(2,2)];
            foot_idx = foot_idx + 4;
        end
    end
end

%%%% Write basic animation data to CSV %%%%%
basic_headers = {'time', 'hinge_x', 'hinge_y', ...
                'stance_x1', 'stance_y1', 'stance_x2', 'stance_y2', ...
                'swing_x1', 'swing_y1', 'swing_x2', 'swing_y2', ...
                'stance_angle', 'swing_angle'};

T_basic = array2table(basic_data, 'VariableNames', basic_headers);
filename_basic = sprintf('walker_animation_basic_%s.csv', datestr(now,'yyyymmdd_HHMMSS'));
writetable(T_basic, filename_basic);
disp(['Basic animation data saved to: ' filename_basic]);

%%%% Write foot data to CSV %%%%%
foot_headers = {};
foot_names = {'stance', 'swing'};
for f = 1:2
    for s = 1:4
        foot_headers = [foot_headers, {[foot_names{f} '_foot_seg' num2str(s) '_x1'], ...
                                      [foot_names{f} '_foot_seg' num2str(s) '_y1'], ...
                                      [foot_names{f} '_foot_seg' num2str(s) '_x2'], ...
                                      [foot_names{f} '_foot_seg' num2str(s) '_y2']}];
    end
end

% Combine all data
all_data = [basic_data, foot_data];
all_headers = [basic_headers, foot_headers];

T_all = array2table(all_data, 'VariableNames', all_headers);
filename_all = sprintf('walker_animation_complete_%s.csv', datestr(now,'yyyymmdd_HHMMSS'));
writetable(T_all, filename_all);
disp(['Complete animation data saved to: ' filename_all]);

%%%% Also save raw state data %%%%%
raw_data = [t_all, z_all];
raw_headers = {'time', 'q1', 'u1', 'q2', 'u2', 'TE', 'xh', 'vxh', 'yh', 'vyh'};
T_raw = array2table(raw_data, 'VariableNames', raw_headers);
filename_raw = sprintf('walker_raw_state_%s.csv', datestr(now,'yyyymmdd_HHMMSS'));
writetable(T_raw, filename_raw);
disp(['Raw state data saved to: ' filename_raw]);
